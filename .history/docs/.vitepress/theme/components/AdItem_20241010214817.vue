<template>
    <div class="amazing" v-if="ads">
        <a class="amazing__link" :style="{ 'pointer-events': item.link ? 'auto' : 'none' }" :href="item.link"
            target="_blank" v-for="item in computedAds">
            <img class="amazing__img" :src="item.img" :title="item.title" :alt="item.title" />
        </a>
    </div>

    <ins v-if="adsense?.slot" class="adsbygoogle" style="display: block" :data-ad-client="adsense.client"
        :data-ad-slot="adsense.slot" data-ad-format="auto" data-full-width-responsive="true"></ins>
</template>
